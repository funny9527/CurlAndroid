...
static CURLcode operate_do(struct GlobalConfig *global,
                           struct OperationConfig *config, char* terminal, char* sec_path)
{
...
CURLcode operate(struct GlobalConfig *config, int argc, argv_item_t argv[], char* terminal, char* path)
{
  CURLcode result = CURLE_OK;
    ...
    if(res) {
      ...
      if(!result) {
        ...
        while(!result && config->current) {
          result = operate_do(config, config->current, terminal, path);
